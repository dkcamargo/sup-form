...
export default class Header extends Component {
  ...
  componentDidMount() {
    window.addEventListener("scroll", () => {
      ...
      if (document.body.scrollTop > shrinkOn || document.documentElement.scrollTop > shrinkOn) {
        this.setState({
          headerId: "header-smaller",
          titleId: "header-title-smaller"
        });
        return;
      } else if (document.body.scrollTop < shrinkOn - 2 || document.documentElement.scrollTop < shrinkOn - 2) {
          this.setState({
          headerId: "header-big",
          titleId: "header-title-big"
        });
